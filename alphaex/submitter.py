...
class Submitter(object):
	...
	Args:
		...
		script_path (str): the slurm array job submission script in the experiment project
	...
	"""
	def __init__(self, clusters, total_num_jobs, script_path, duration_between_two_polls=60, repo_url=None):
		# sanity check
		...
		for cluster in clusters:
			...
			if 'exp_results_from' in cluster and 'exp_results_to' in cluster:
				if cluster['exp_results_from'].__len__() != cluster['exp_results_to'].__len__():
					print("length of list exp_results_from must equal to length of list exp_results_to")
					exit(1)
			elif ('exp_results_from' not in cluster and 'exp_results_to' in cluster) or ('exp_results_from' in cluster and 'exp_results_to' not in cluster):
				print("exp_results_from and exp_results_to must be both specified or unspecified")
				exit(1)
			else:
	...
	def submit_jobs(self, num_jobs, cluster_name, project_root_dir):
		bash_script = "ssh %s 'cd %s; sbatch --array=%d-%d %s'" % (
			cluster_name, project_root_dir, self.starting_job_num, self.starting_job_num + num_jobs - 1, self.script_path
		)
		print(bash_script)
		myCmd = os.popen(bash_script).read()
		print(myCmd)
				...
				if finish_submitting:
					if num_current_jobs == 0:
						for i in range(len(cluster['exp_results_from'])):
							...
							cluster['name'], cluster['exp_results_from'][i], cluster['exp_results_to'][i])
							print(bash_script)
							myCmd = os.popen(bash_script).read()
							print(myCmd)
					...
					finish_submitting = self.submit_jobs(
						...
						cluster['name'], cluster['project_root_dir']
					)
					if finish_submitting:
